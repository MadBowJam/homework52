...
import { writeFile } from 'fs/promises';
 ...
 async function writeFileAsync(filename, content) {
  try {
    // Запис вмісту в файл асинхронно
    await writeFile(filename, content);
    
    // Логування успішного запису
    console.log('Файл успішно записано');
  } catch (error) {
    // Логування помилок
    console.error('Помилка при записі файлу:', error);
    return error; // Повертаємо помилку
  }
}
...
async function readFileAsync(filename) {
  try {
    // Асинхронне читання вмісту файлу
    const content = await readFile(filename, 'utf8');
    
    // Логування успішного читання
    console.log('Файл успішно прочитано:', content);
    
    // Повернення вмісту файлу
    return content;
  } catch (error) {
    ...
    return null; // Повернення null у випадку помилки
  }
}
...
async function deleteFileAsync(filename) {
  try {
    ...
    // Обробка помилок видалення файлу
    if (error.code === 'ENOENT') {
      console.error('Файл не існує:', filename);
    } else {
      console.error('Помилка при видаленні файлу:', error);
